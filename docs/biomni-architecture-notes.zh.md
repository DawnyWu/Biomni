...
> **先定义 oracle。** 术语来自软件测试里的 *test oracle*：**一个能自动判定「结果对不对」的裁判**。它是 agent 迭代的前提——没有裁判，模型就拿不到「刚才那步错了」的信号，也就无从改进。
>
> 注意一个术语歧义：TDC（Therapeutics Data Commons）里的 `Oracle` 是**另一个意思**——分子性质的打分函数（QED 类药性、SA 合成可及性、GSK3B 活性概率），Biomni 的 `pharmacology.py:97` 就 `from tdc import Oracle`，ToolUniverse 也包了一个 `TDC_predict_oracle_score`。那个是「用模型当代理评分器」，本节说的是「正确性裁判」。两者直觉相通（都是能给出评判的黑盒），但不是同一个东西。
...
「机器可读的目录」指的就是 `env_desc.py` 那两个 dict 的形态：把「你有什么数据、装了什么软件」写成结构化清单，每条配一句领域解释，让模型不用打开文件就知道该用哪个。三个要点：

- **结构化**（dict / JSON / YAML），不是散文 README——因为检索器要能枚举它、按条切片
- **每条说「这是什么 + 什么时候用」**，不是说格式。写「一个 GNN 模型」没用，写「秒级估算形成能，可替代廉价 DFT 预筛」才有用
- **粒度等于可被选择的单位**——一个文件一条、一个包一条，因为检索器是按条选的

材料方向照着写就是：

```python
data_lake_dict = {
    "mp_summary_2024.parquet": "Materials Project 全库摘要（形成能、带隙、体积模量），用于候选材料初筛",
    "xrd_reference_patterns.parquet": "COD 衍生的参考 XRD 图谱，用于实验图谱定相比对",
}
library_content_dict = {
    "pymatgen": "[Python Package] 晶体结构操作、相图构建、VASP/QE 输入输出解析",
    "chgnet": "[Python Package] 预训练机器学习势，秒级估算形成能与松弛结构，可替代廉价 DFT 做预筛",
}
```

之所以说不可替代：agent 循环可以照抄，工具甚至可以自动生成（ToolUniverse 的 `UnifiedToolGenerator` 就在做），但「affinity capture-MS 是测 PPI 的」这句话必须懂行的人来写——这是整条链上唯一不可自动化的环节。

（一个范围限定：走 [OPTIMADE](./optimade-materials-data-notes.zh.md) 的数据源是**自我描述**的，agent 运行时拉 `providers.json` 和 `/info/structures` 就能自己搞清楚有什么可查，不必手写。手写目录的价值主要集中在**本地数据和本地软件**上。）

**2. 把不可验证的领域判断固化成 know-how 文档或 curated 工具**（对应前提三）——例如「XRD 定相该用什么容差」「什么时候必须做 Rietveld 精修」，这些是模型自己想不出、也不会被任何报错纠正的东西。

**3. 想清楚你的 oracle 是什么**（对应前提三）——能不能构造一批有标准答案的题？

没有 oracle 会陷入三个困境：改了 prompt 不知道有没有变好、agent 无法自我迭代、也没法说服别人。Biomni 造的那个 oracle 就是 Biomni-Eval1（433 实例 / 10 任务），纯手工。

**材料方向在这一条上比生物有优势**：留出一批有实验值的形成能/带隙做测试集就是现成的数值 oracle；已知结构的 XRD 图谱可以反过来验证定相是否正确。相比「生成 32 个能最大化扰动效应的基因」这类题，材料领域天然有更多可量化的标准答案。
...
> **一句话**：它让 agent 的每一轮变成「在同一个 Jupyter kernel 里加一个 cell」，而不是「重新跑一遍脚本」。
>
> 好处随任务步数和数据规模放大，代价基本固定。所以长时程、数据密集的任务收益极大；「查三个 API 然后总结」则毫无价值，标准 function-calling 更简单更安全。
